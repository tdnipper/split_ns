...
========================================================================================
    Workflow:
        1.  Validate FASTQ format              (fq lint)
        2.  QC on raw reads                    (FastQC)
        3.  Extract UMIs from raw reads        (UMI-tools extract)
        4.  Merge paired-end reads             (BBMerge)
        5.  Trim flanking NS sequences         (Cutadapt)
        6.  QC on trimmed reads                (FastQC)
        7.  Build Bowtie index from sgRNA lib  (Bowtie v1)
        8.  Align trimmed reads                (Bowtie v1)
        9.  Sort & index BAM                   (SAMtools)
        10. Deduplicate reads by UMI           (UMI-tools dedup)
        11. Count sgRNAs                       (MAGeCK count)
        12. Rank genes by enrichment/depletion (MAGeCK test)
        13. Aggregate QC reports               (MultiQC)
========================================================================================
*/
...
workflow {
    ...
    //   [ [id, condition, single_end:false], [ /path/to/R1.fastq.gz, /path/to/R2.fastq.gz ] ]
    ...
    // -- 8. Convert library TXT to FASTA, then build Bowtie index ---------------
    ...
    // -- 9. Align reads to sgRNA library ----------------------------------------
